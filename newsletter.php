...
function tokenHash(string $v): string {
...
function ipBin(): ?string {
    return isset($_SERVER['REMOTE_ADDR']) ? @inet_pton($_SERVER['REMOTE_ADDR']) ?: null : null;
}
...
function verifyUnsubscribeToken(array $site, string $token): ?array {
    ...
    $check = hash_hmac(
        'sha256',
        $id . '|' . $email,
        (string)($site['unsubscribe_secret'] ?? 'change-me')
    );
...
function buildEmailHtml(
    ...
    if ($unsubscribeUrl !== null) {
        $footerLegal .= 'Ricevi questa email perché ti sei iscritto alla newsletter ufficiale. Puoi revocare il consenso in qualsiasi momento: <a href="' . htmlspecialchars($unsubscribeUrl, ENT_QUOTES, 'UTF-8') . '">Annulla la tua iscrizione qui</a>.';
    } else {
...
function buildUnsubscribeEmailHtml(array $site): string {
    return buildEmailHtml(
        $site,
        'Iscrizione alla newsletter cancellata',
        'Ci dispiace vederti andare via',
        [
            'La tua iscrizione è stata cancellata correttamente e non riceverai più newsletter.',
...
function sendSmtpMail(
    ...
    $expect = static function ($line, string $code): bool {
        return str_starts_with((string)$line, $code);
    };

    if (!$expect($read($fp), '220')) {
        fclose($fp);
        return false;
    }
...
function logEvent(PDO $pdo, int $subscriberId, string $eventType, ?string $note = null): void {
    $stmt = $pdo->prepare(
        'INSERT INTO icdm_subscriber_events (subscriber_id, event_type, ip_address, note) VALUES (:subscriber_id, :event_type, :ip, :note)'
    );
...
if ($action === 'subscribe' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!filter_var($email, FILTER_VALIDATE_EMAIL) || !$consent || $name === '') {
        header('Location: contatti.html?newsletter=invalid');
        exit;
    }
    ...
    $stmt = $pdo->prepare(
        ...
        ON DUPLICATE KEY UPDATE 
            ...
            status = \'pending\',
            confirm_token_hash = VALUES(confirm_token_hash),
            consent_marketing = 1,
    ...
    $stmt->execute([
        ':email' => $email,
        ':full_name' => $name ?: null,
        ':confirm_hash' => tokenHash($confirmToken),
        ':unsubscribe_hash' => tokenHash($unsubscribeToken),
        ':consent_at' => now(),
        ':confirm_expires_at' => $expires,
        ':source_page' => 'website',
        ':ip' => ipBin(),
        ':ua' => substr((string)($_SERVER['HTTP_USER_AGENT'] ?? ''), 0, 255),
        ':privacy_version' => 'privacy-newsletter-2026-05-16',
        ':privacy_url' => rtrim($config['site']['base_url'], '/') . '/privacy.html',
        ':consent_text' => 'Ho letto l\'informativa privacy e acconsento a ricevere comunicazioni email sul libro, aggiornamenti editoriali, anticipazioni e notizie sulla pubblicazione.',
    ]);
    ...
    sendTextMail(
        $config['site']['ops_email'],
        'Nuova richiesta iscrizione newsletter',
        "Nuova richiesta iscrizione: $email",
        $config['site'],
        $config['smtp'] ?? [],
        null
    );
    ...
    if (!$okSubscriber) {
        header('Location: contatti.html?newsletter=mail-failed');
        exit;
    }

    header('Location: contatti.html?newsletter=check-email');
    exit;
}
...
if ($action === 'confirm' && isset($_GET['token'])) {
    ...
    if ($stmt->rowCount() > 0) {
        ...
        if (is_array($subscriber)) {
            ...
            sendTextMail(
                (string)$subscriber['email'],
                'Benvenuto nella newsletter - Il Custode dei Miracoli',
                $welcomeText,
                $config['site'],
                $config['smtp'] ?? [],
                $welcomeHtml
            ...
            sendTextMail(
                $config['site']['ops_email'],
                'Iscrizione confermata newsletter',
                'Iscrizione confermata: ' . (string)$subscriber['email'],
                $config['site'],
                $config['smtp'] ?? [],
                null
            );
    ...
    if ($stmt->rowCount() > 0) {
        header('Location: iscrizione-completata.html');
        exit;
    }
...
if ($action === 'unsubscribe_token' && isset($_GET['token'])) {
    ...
    if ($decoded === null) {
        echo 'Link di cancellazione iscrizione non valido.';
        exit;
    }
    ...
    if ($stmt->rowCount() > 0) {
        ...
        $goodbyeText = 'La tua cancellazione alla newsletter è confermata. Ci dispiace vederti andare via.';
        $goodbyeHtml = buildUnsubscribeEmailHtml($config['site']);

        sendTextMail(
            (string)$decoded['email'],
            'Cancellazione iscrizione confermata - Il Custode dei Miracoli',
            $goodbyeText,
            $config['site'],
            $config['smtp'] ?? [],
            $goodbyeHtml
        ...
        sendTextMail(
            $config['site']['ops_email'],
            'Cancellazione iscrizione dalla newsletter confermata',
            'Cancellazione iscrizione dalla newsletter confermata tramite link: ' . (string)$decoded['email'],
            $config['site'],
            $config['smtp'] ?? [],
            null
        );
...
if ($action === 'unsubscribe' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($stmt->rowCount() > 0) {
        $goodbyeText = 'La tua iscrizione dalla newsletter è stata cancellata. Ci dispiace vederti andare via.';
        $goodbyeHtml = buildUnsubscribeEmailHtml($config['site']);
        ...
        sendTextMail(
            $config['site']['ops_email'],
            'Cancellazione iscrizione dalla newsletter confermata',
            "Cancellazione iscrizione dalla newsletter confermata tramite form: $email",
            $config['site'],
            $config['smtp'] ?? [],
            null
        );
    ...
    header('Location: contatti.html?newsletter=not-active');
    exit;
}
